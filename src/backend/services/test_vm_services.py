...
class TestVendingServices:
    """Test services."""

    def __init__(self: "TestVendingServices") -> None:
    ...
    def query_all_machines(self: "TestVendingServices") -> json:
        """Query all machines."""
        all_machines: json = requests.get(self.all_machines_url).json()
        return all_machines

    def add_machine(self: "TestVendingServices", name: str, location: str) -> requests:
        """Add machine."""
        test_json: json = {"name": name, "location": location}
        response: requests = requests.post(url=self.add_machine_url, json=test_json)
        return response

    def add_machine_if_empty(self: "TestVendingServices", name: str, location: str) -> json:
