...
class StockTestServices:
    ...
    def add_stock(self: "StockTestServices", vm_id: int, product: str, quantity: int) -> requests:
        """Add stock."""
        test_json: json = {"vm_id": vm_id, "product": product, "quantity": quantity}
        response: requests = requests.post(url=self.add_stock_url, json=test_json)
        return response

    def add_stock_if_empty(self: "StockTestServices", vm_id: int, product: str, quantity: int) -> json:
        """Add stock if empty."""
        stock_id: int = 0
        if len(self.query_all_stocks()) == 0:
            stock_id: int = self.add_stock(vm_id, product, quantity).json()["id"]
        return stock_id
